...
| Concept                   | Category                  |
| -------                   | --------                  |
| `ArithmeticIntervalType`  | `continuous || integral`  |
...
```c++
enum class IntervalOrder; // see below for the list of values
```
...
less than `B` (apart from the special case of `both_empty`); a positive value
indicates that `A` will compare greater than `B`.
...
| `equal`              | 0      | `***`        | `A` and `B` are the same (and not empty)                         |

...
void test_crow_interval_continuous_interval_set_construct_insert_erase() {
    ...
    TRY((set = {{1,2,"()"}}));                        TRY(str = set.str());  TEST_EQUAL(str, "{(1,2)}");
    ...
    TRY((set.insert({20,30,"()"})));  TRY(str = set.str());  TEST_EQUAL(str, "{[10,30)}");
    TRY((set.erase({5,10,"()"})));    TRY(str = set.str());  TEST_EQUAL(str, "{[10,30)}");
    TRY((set.erase({5,10})));         TRY(str = set.str());  TEST_EQUAL(str, "{(10,30)}");
    TRY((set.erase({12,14})));        TRY(str = set.str());  TEST_EQUAL(str, "{(10,12),(14,30)}");
    TRY((set.erase({16,18,"()"})));   TRY(str = set.str());  TEST_EQUAL(str, "{(10,12),(14,16],[18,30)}");
    TRY((set.insert({9,11})));        TRY(str = set.str());  TEST_EQUAL(str, "{[9,12),(14,16],[18,30)}");
    TRY((set.insert({29,31,"()"})));  TRY(str = set.str());  TEST_EQUAL(str, "{[9,12),(14,16],[18,31)}");

}

void test_crow_interval_continuous_interval_set_operations() {
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        for (int y = 0; y <= max_value + 1; ++y) {
            ...
            if ((i_test != i_expect) || (u_test != u_expect) || (d_test != d_expect) || (sd_test != sd_expect)) {
                for (int j = 0; j < 2; ++j)
                    std::cout << "... " << format_range(vec[j]) << " => " << set[j] << "\n";
                std::cout << "... x=" << x << "\n";
            }
        }
...
void test_crow_interval_continuous_interval_set_hashing() {
